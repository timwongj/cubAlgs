module.exports = (function() {
  ...
  var Cub = function() {
    ...
    for (var i = 0; i < faceColors.length; i++) {
      for (var j = 0; j < faceColors[i].face.length; j++) {
        self[faceColors[i].face[j]] = faceColors[i].color;
      }
    }
  };
   ...
   * @param {string} movesString - The moves to perform.
   */
  Cub.prototype.perform = function(movesString) {
    var stickers, temp, moves = movesString.split(' '), moveCount = 0, self = this;
    for (var i = 0; i < moves.length; i++) {
      var face = (moves[i].substr(0, 1) === '[') ? moves[i].substr(1, moves[i].length - 2) : moves[i].substr(0, 1);
      var dir = (moves[i].length === 1) ? 1 : ((moves[i].substr(1, 2) === '2') ? 2 : -1);
      switch(face) {
      ...
      for (var j = 0; j < stickers.length; j++) {
        switch(dir) {
          case 1:
            temp = self[stickers[j][0]];
            self[stickers[j][0]] = self[stickers[j][1]];
            self[stickers[j][1]] = self[stickers[j][2]];
            self[stickers[j][2]] = self[stickers[j][3]];
            self[stickers[j][3]] = temp;
            break;
          case 2:
            temp = self[stickers[j][0]];
            self[stickers[j][0]] = self[stickers[j][2]];
            self[stickers[j][2]] = temp;
            temp = self[stickers[j][1]];
            self[stickers[j][1]] = self[stickers[j][3]];
            self[stickers[j][3]] = temp;
            break;
          case -1:
            temp = self[stickers[j][0]];
            self[stickers[j][0]] = self[stickers[j][3]];
            self[stickers[j][3]] = self[stickers[j][2]];
            self[stickers[j][2]] = self[stickers[j][1]];
            self[stickers[j][1]] = temp;
            break;
        }
      }
    }
  ...
  return {
     ...
     * @returns {*}
     */
    validateAlg: function(alg, algCase) {
      if (!(alg && algCase)) {
        return Promise.reject('alg and case are required');
      } else {
        var solvedCub = new Cub();
        var cub = new Cub();
        for (var key in algCase) {
          if (algCase.hasOwnProperty(key)) {
            cub[key] = solvedCub[algCase[key]];
          }
        }
        var moves = cub.perform(alg);
        if (cub.isSolved()) {
          return Promise.resolve(moves);
        } else {
          return Promise.reject('alg does not solve case');
        }
      }
    }
